...
public class GuiGC extends Screen
{
    ...
    @Override
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float renderPartials)
    {
        modified = false;
        background.draw(guiGraphics);
        spawnSection.drawSection(guiGraphics, 40, 40, 40);
        deleteSection.drawSection(guiGraphics, 255, 0, 0);
        for (EditBox field : textFields)
        {
            field.render(guiGraphics, mouseX, mouseY, renderPartials);
        }

        for (AbstractWidget current : buttons)
        {
            current.render(guiGraphics, mouseX, mouseY, renderPartials);
        }

        if (tile.isErrored())
        {
            Component error = Component.literal("Error: " + tile.getErrorMessage());
            guiGraphics.drawWordWrap(font, error, guiX + 3,
            ...
            sec.drawSection(guiGraphics, 255, 0, 0, 100);
        }

        if (dragging)
            draggedBlock.render(guiGraphics, mouseX, mouseY, renderPartials);
        ...
        for (EditBox current : textFields)
        {
            ...
            {
                guiGraphics.renderTooltip(font, Component.literal(fieldToString.get(current)), current.getX() + current.getWidth(), current.getY() + current.getHeight());
            }
        }
        ...
        for (AbstractWidget current : buttons)
        {
            ...
            if (current instanceof ExpressionBlock block)
            {
                ...
                {
                    guiGraphics.renderTooltip(font, Component.literal(Evaluations.infoMap.get(block.expression.evaluation).tooltip), guiX - 7,
        ...
        if (deleteSection.isPointWithinIncl(mouseX, mouseY))
            guiGraphics.renderTooltip(font, Component.literal(DELETE_SECTION), deleteSection.x, deleteSection.y);
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int mouseButton)
    {
        ...
        for (int i = buttons.size() - 1; i >= 0; i--)
        {
            ...
            {
                if (buttons.get(i) instanceof EditBox editBox)
                {
                    setFocused(editBox);
                }
                return true;
    ...
    public void backOne()
    {
        for (int i = 0; i < textFields.length; i++)
            if (textFields[i].isFocused())
            {
                ...
                EditBox nextField;
                if (i - 1 >= 0)
                    nextField = textFields[i - 1];
                else
                    nextField = textFields[textFields.length - 1];
